...
const reloadDraw = async () => {
  ...
  const res = await request.post(api.picture_resetting, {
    pictureId: useStore.activeHistoryItem.active.pictureId,
  });
  if (res.code !== 200) {
    loading.value = false;
    return ElMessage({
  ...
  useStore.activeHistoryItem.active.isFail = false;
  //todo:
...
const madePicture1 = async (position, k) => {
  try {
    ...
    const res = await request.post(api.picture_onePicture, {
      parentPictureId: useStore.activeHistoryItem.active.pictureId,
    ...
    userStore.saveResiduePictureQuantity(res.data.residuePictureQuantity);
    useStore.activeHistoryItem.active = {
...
onMounted(() => {
  if (useStore.active.pictureId != null) {
    openTimer(useStore.active);
  }
});
